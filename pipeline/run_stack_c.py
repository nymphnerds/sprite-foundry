"""
Pass A — Stack C Only: Reference Control
JuggernautXL + pixel-art-xl @ 0.75, higher cfg (8.5), dpmpp_2m/karras, turnaround prompting
...
NEGATIVE_PROMPT = (
    ...
    "cropped, cut off, inconsistent design, different characters"
)


def make_workflow(direction_prompt: str, seed: int) -> dict:
    """Stack C: JuggernautXL + pixel-art-xl @ 0.75, dpmpp_2m/karras, cfg 8.5, 35 steps."""
    return {
        "1": {
...
def make_contact_sheet(images, run_id, output_path):
    ...
    ox, oy = 10, 10
    draw.text((ox, oy), "PASS A — Stack C: Reference Control", fill=ACCENT, font=font_lg)
    oy += 24
    ...
    for line in [
        f"Run ID:     {run_id}",
        f"Stack:      C — Reference Control (JuggXL + pixel-art-xl @ 0.75, cfg 8.5, dpmpp_2m/karras)",
...
def run():
    ...
    sheet = out_dir / "contact_sheet_C.png"
    ...
    with open(out_dir / "recipe.json", "w") as f:
        json.dump({
            ...
            "seed": SEED,
            "prompt_strategy": "turnaround sheet + consistency emphasis",
            "subject_prompt": SUBJECT_PROMPT,
            "style_suffix": STYLE_SUFFIX,
            "negative": NEGATIVE_PROMPT,
        }, f, indent=2)

    print(f"\n  Stack C complete: {len(images)}/8 directions")
