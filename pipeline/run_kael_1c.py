"""
Phase 1C.1 — Second Character Pressure Test: Kael Morrow

Stack A v2 law, identical pipeline to Sera Vale run.
Same: portrait aspect, isolation prompts, raw-source gate, mechanical checks.
Different: subject prompt (harder silhouette), fresh seed.
"""
...
SPRITE_TARGET = 48
SEED = 616161  # fresh seed for Kael
...
# Kael Morrow subject prompt — stronger single-subject isolation
SUBJECT_PROMPT = (
    "solo, 1man, single character only, one person, "
    "a man named Kael Morrow, ship engineer and salvage specialist, "
    "heavy dark charcoal grey half-coat with asymmetric hem longer on left side, "
    "reinforced metal shoulder pad on right shoulder, "
    "dark olive green high collar undershirt, "
    "wide leather tool belt low on left hip with wrench and tools, "
    "welding mask pushed up on top of head with dark lens, "
    "thick heavy soled boots with reinforced toe caps, dark brown, "
    "dark fingerless work gloves, "
    "shaggy medium length dark auburn hair swept back, "
    "lean broad shouldered build, light warm skin tone, "
    "standing idle neutral pose, work-worn appearance"
)
...
# Same strong anti-multi-subject negatives + Kael-specific forbidden drift
NEGATIVE_PROMPT = (
    ...
    "flowing cape, full cape, weapon in hand, sword, gun, "
    "helmet, full helmet, backpack, large pack, "
    "armor plating, full armor, glowing effects, magic effects, "
    "bright colors, clean pristine appearance, symmetrical coat, "
    "busy background, cropped, cut off, partial body"
)
...
def make_raw_contact_sheet(raw_images, run_id, output_path):
    ...
    ox, oy = 10, 10
    draw.text((ox, oy), "RAW SOURCE INSPECTION — Kael Morrow 1C", fill=ACCENT, font=font_lg)
    oy += 24
    ...
    oy += 6
    draw.text((ox, oy), f"Run: {run_id} | Check: single subject, centered, no reflection/companion", fill=TEXT, font=font_sm)
...
def make_pixel_contact_sheet(pixel_images, run_id, output_path):
    ...
    ox, oy = 10, 10
    draw.text((ox, oy), "Phase 1C — Kael Morrow — Stack A v2 (48x48)", fill=ACCENT, font=font_lg)
    oy += 24
    ...
    draw.line([(ox, oy), (total_w - 10, oy)], fill=GRID)
    oy += 6
    draw.text((ox, oy), f"Run: {run_id} | bg removed, portrait 576x768, seed {SEED}", fill=TEXT, font=font_sm)
...
def run():
    ...
    pixel_sheet = out_dir / "contact_sheet_kael.png"
    ...
    print(f"\n  Kael Morrow 1C complete: {len(pixel_images)}/8 directions")
