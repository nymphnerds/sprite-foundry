"""
Pass A — Stack B Only: Stylized + Cleanup
JuggernautXL + PixelArtRedmond-Lite64 @ 0.7 + DetailTweakerXL @ 0.4
...
def make_workflow(direction_prompt: str, seed: int) -> dict:
    """Stack B: JuggernautXL + PixelArtRedmond @ 0.7 + DetailTweaker @ 0.4."""
    return {
        "1": {
...
def make_contact_sheet(images, run_id, output_path):
    ...
    ox, oy = 10, 10
    draw.text((ox, oy), "PASS A — Stack B: Stylized + Cleanup", fill=ACCENT, font=font_lg)
    oy += 24
    ...
    for line in [
        f"Run ID:     {run_id}",
        f"Stack:      B — Stylized + Cleanup (JuggXL + PixelArtRedmond @ 0.7 + DetailTweaker @ 0.4)",
...
def run():
    ...
    sheet = out_dir / "contact_sheet_B.png"
    ...
    with open(out_dir / "recipe.json", "w") as f:
        json.dump({
            ...
            "loras": "PixelArtRedmond-Lite64 @ 0.7 + DetailTweakerXL @ 0.4",
            "trigger": "PixArFK",
    ...
    print(f"\n  Stack B complete: {len(images)}/8 directions")
